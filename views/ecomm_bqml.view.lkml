    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `interno-davinci-analitica.thelook_ecommerce.order_items` as oi
                  JOIN `interno-davinci-analitica.thelook_ecommerce.inventory_items` as inv ON inv.id = oi.inventory_item_id
      ...
      JOIN (SELECT user_id as user_id1
            ...
            FROM `demosaludfamiliar.ecomm.events`
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `demosaludfamiliar.ecomm.kmeans_model5`,
        (SELECT * FROM
